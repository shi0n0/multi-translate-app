...
- (6/23)  
 結構悩んで色々やってみたのですが、どうしても治らないので個別に細かい設定欄を追加しました。  
 ChatGPTの方では問題なく生成されるのでmodelの問題なのかAPIの仕様なのかどうか...  
 どうやら今の詳細に設定を書いて物語を生成する方法ではうまくいっているのでしばらくはこちらのまま進めて、余裕が出てきたら文章での生成に関しても模索してみようかと思います。  
<br>

- (6/24)  
 プロンプト的にストーリーを作るというより設定を読み上げているだけなので、きちんとストーリーが作られるようにプロンプトを変更しました。  
 長さや短編ストーリーと指定すると文字数も足りないのでそうなるみたいですね。今後も変更してくと思いますが、とりあえず長編に変更して、tokenを1000に拡張したらある程度上手くいくようになりました。  
 <br>

- New!(6/25)  
 これにてとりあえずこちらの開発は一旦終了にしようと思います。  
 理由は他の開発もしてみたくなったからです。GPTAPIを触ってみるという目的は達成できたのでとりあえずこれにて。  
 でも、完全に終了するというわけではないので、気が向いたらこちらの開発も息抜きにするかもしれません。
 <br>
